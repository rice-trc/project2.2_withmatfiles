function ms_plot(t,y,u,freq,idof,benchmark,A,dataname,savefig,fpath)
...
if P > 1
figure;
per = (y(:,1:end-1,r,idof)-y(:,end,r,idof)) / rms(y(:,1,r,idof));
...
if savefig
    for i=1:length(hfig)
        ...
        set(h, 'Color', 'w');
        figname = sprintf('%s/b%d_A%g_%s_%s',fpath,benchmark,A,dataname,fname);
        export_fig(h, figname,'-png');
    end

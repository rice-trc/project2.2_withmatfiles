...
Om_vec = linspace(Om_s, Om_e, 10);  % Vector of excited frequencies
R = length(Om_vec);
P = 10;
...
NTrans = 10*Nt;  % Add two periods before the start
...
for A = Avec
    ...
    nper = 5;
    for i = 1:length(Om_vec)
        ymax = max(yper(:,end-nper:end,i));
        ymin = min(yper(:,end-nper:end,i));

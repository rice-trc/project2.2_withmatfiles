function amp = arrow_amplitude(A1,A2,T,P)
% linear increase of amplitude from A1 to A2 during on period and then
...
% Or for testing the system. In that case, P=1 is sufficient
% Note that the frequency content is destroyed, because the signal is no
% longer periodic
%
% INPUTS
%  A1,A2: start and end ampltiudes.
%  T: Period time, eg T = 1/f0
%  P: number of periods. Default P=1
...
% plot(freq(1:ndft),db(abs(U2(1:ndft))))
% xlabel('Frequency (Hz)')
% ylabel('Magnitude (dB)')
% legend('original','scaled')
